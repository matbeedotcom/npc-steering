...
|---|---|---|---|
| 1 | Rewrite `decisions.py` with paper-anchored coefficient structure (sign + interaction terms from Tables 1, 6) | `lib/decisions.py` | medium |
| 2 | Add arousal-gating (>+0.18) to appraisal modulators per Table 2's "only-when-aroused" finding | `lib/appraisal.py` | medium |
| 3 | Add the SAM↔V/A mapping note to `design.md` §3.1 | `notes/design.md` | low |
| 4 | Add citation paragraph to `writeup.md` "Limitations / future work" section, framing this paper as the empirical anchor for personality-arousal interaction and noting that decay τ remain biometric-grounding gap | `notes/writeup.md` | high |
| 5 | When/if we tune `anxious_new_driver`: per Table 7, high-N + low-experience drivers fail to learn from events. Could implement as reduced re-appraisal coupling κ for that persona | `lib/agent.py` | low |
